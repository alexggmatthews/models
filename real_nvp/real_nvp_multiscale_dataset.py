...
def conv_ch_aff_coupling(input_, dim, name,
    ...
    with tf.variable_scope(name) as scope:
        ...
        if change_bottom:
            ...
            input_, canvas = tf.split(3, 2, input_)
        else:
            #canvas, input_ = tf.split(input_, 2, 3)
            canvas, input_ = tf.split(3, 2, input_)
        shape = input_.get_shape().as_list()
        batch_size = shape[0]
        height = shape[1]
        width = shape[2]
        ...
        shift, log_rescaling = tf.split(3, 2, res)
        scale = variable_on_cpu(
            "scale", [],
    ...
    if scale_idx < (n_scale - 1):
        ...
        if FLAGS.recursion_type > 1:
            ...
            if FLAGS.recursion_type > 2:
                ...
                res_1, res_2 = tf.split(3, 2, res)
                log_diff_1, log_diff_2 = tf.split(3, 2, log_diff)
